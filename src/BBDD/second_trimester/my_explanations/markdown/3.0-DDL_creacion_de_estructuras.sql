...
USE EJEMPLO; /* INICIALIZA EL USO/CONEXIÓN CON UNA BASE DE DATOS */
...
/* CALCULO DE LA EDAD DE UNA PERSONA QUE NACIÓ EL 19-01-1973, REDONDEANDO LA EDAD A DOS DECIMALES */
...
SELECT CONCAT(
    UCASE(DAYNAME(NOW())), ', ', 
    DAYOFMONTH(NOW()), ' de junio de ', 
    YEAR(NOW()), ' - QUEDAN ', 
    365 - DAYOFYEAR(NOW()), ' DIAS PARA FIN DE AÑO QUE SERÁ - ', 
    UPPER(DAYNAME('2020-12-31'))
) AS MENSAJE;

SELECT CONCAT_WS('-', CONCAT(LEFT(NOMBRE,1), LEFT(APEL1,1), LEFT(APEL2,1)), LEFT(IDCLI,3)) AS INICIALES 
FROM ENVIOS; /* LEFT(campo, n) => N es el num de caracteres a extraer => J-P-L-123 */
